...
int ec_detect_slaves()
{
   return ecx_detect_slaves(&ecx_context);
}


int ec_read_all_states(){
   int wkc;
   uint16_t rval;
   return ecx_BRD(ecx_context.port, 0, ECT_REG_ALSTAT, sizeof(rval), &rval, EC_TIMEOUTRET);
}
/** Map all PDOs in one group of slaves to IOmap with Outputs/Inputs
 * in sequential order (legacy SOEM way).
 *
